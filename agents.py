...
def prompt_ai(messages):
    ...
    if tool_calls:
        ...
        # [ChatCompletionMessageToolCall(id='call_l9ME6hEyPhmRMLM4lckfia7n', function=Function(arguments='{"task_name":"Go to school by noon","due_on":"2024-12-30"}', name='create_asana_task'), type='function')]
        # Next, for each tool the AI wanted to call, call it and add the tool result to the list of messages
        for tool_call in tool_calls:
            function_name = tool_call.function.name #'create_asana_task'
            function_to_call = available_functions[function_name]
            function_args = json.loads(tool_call.function.arguments) #'{"task_name":"Go to school by noon","due_on":"2024-12-30"}'
            function_response = function_to_call(**function_args) 
            #**fancy way to call an object like available_functions turn 
            #them into arguments for function call
